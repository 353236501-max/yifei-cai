...
配置中的 `DB` 用于错题文字记录。D1 数据库不存在时由 Wrangler 创建。R2 图片存储暂时不写入部署配置，因为部分 Cloudflare 账号尚未启用 R2；网页、知识讲解、变式练习、API 和文字错题记录不依赖 R2。启用 R2 后，可在 `wrangler.jsonc` 中加入 `BUCKET` 绑定并重新部署，图片上传才会启用。
